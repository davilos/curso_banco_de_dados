...
-- Em anos
-- SELECT DATE_PART('year', '2019-01-01'::date) - DATE_PART('year', '2011-10-02'::date)
-- 	AS Em_anos;
...
-- Em semanas
-- SELECT TRUNC(DATE_PART('day', '2019-01-01'::timestamp - '2011-12-22'::timestamp) / 7)
-- 	AS Em_semanas;
...
-- Transformando para português
-- SHOW lc_time;

-- SET lc_time='pt_BR.UTF8';

-- SELECT TO_CHAR(CURRENT_TIMESTAMP, 'TMDay') AS Dia_da_Semana;
-- SELECT TO_CHAR(CURRENT_TIMESTAMP, 'TMMonth') AS Dia_da_Semana;
...
-- SELECT EXTRACT(MILLISECOND FROM CURRENT_TIMESTAMP) AS MiliSegundos;

-- Convertendo de segundos para hora
-- SELECT TO_CHAR((67691.859651 || 'seconds')::interval, 'HH24:MM:SS') AS Tempo_Total;
